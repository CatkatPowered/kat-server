...
public class KatCertUtil {
  ...
  public static SslContextFactory.Server getSslContextFactory() {
    if (
      KatServer.KatConfigAPI
        .<Boolean>getConfig(
          KatConfigNodeConstants.KAT_CONFIG_NETWORK_CUSTOM_CERT_ENABLED
        )
        .get()
    ) {
      // 自定义证书
      ...
      sslContextFactory.setKeyStorePath(
        KatServer.KatConfigAPI
          .<String>getConfig(
            KatConfigNodeConstants.KAT_CONFIG_NETWORK_CUSTOM_CERT_PATH
          )
          .get()
      );
      sslContextFactory.setKeyStorePassword(
        KatServer.KatConfigAPI
          .<String>getConfig(
            KatConfigNodeConstants.KAT_CONFIG_NETWORK_CUSTOM_CERT_PASSWORD
          )
          .get()
      );
  ...
  public static KeyStore getKeyStore() {
    if (keyStore == null) {
      String password = KatServer.KatConfigAPI
        .<String>getConfig(
          KatConfigNodeConstants.KAT_CONFIG_NETWORK_SELFGEN_CERT_PASSWORD
        )
        .get();
      String alias = KatServer.KatConfigAPI
        .<String>getConfig(
          KatConfigNodeConstants.KAT_CONFIG_NETWORK_SELFGEN_CERT_ALIAS
        )
        .get();
      ...
      try {
        ...
        Certificate CAcertificate = new JcaX509CertificateConverter()
          ...
          .getCertificate(
            createCertificate("CN=CA", "CN=CA", publicKey, privateKey)
          );
        Certificate CLcertificate = new JcaX509CertificateConverter()
          .setProvider(BouncyCastleProvider.PROVIDER_NAME)
          .getCertificate(
            createCertificate("CN=CA", "CN=CA", publicKey, privateKey)
          );
  ...
  private static X509CertificateHolder createCertificate(
    String dn,
    String issuer,
    PublicKey publicKey,
