  ...
  public static void KatNetworkIncomeHandler(WsConfig ws) {
    ...
    ws.onConnect(wsConnectContext -> {
      ...
      wsConnectContext.send(
        gson.toJson(
          ServerDescriptionPacket
            .builder()
            .version(KatMiscConstants.KAT_SERVER_VERSION)
            .build()
        )
      );
    ...
    ws.onMessage(wsMessageContext -> {
      String type = gson
        .fromJson(wsMessageContext.message(), BasePacket.class)
        .getType();
      switch (type) {
        case KatPacketTypeConstants.MESSAGE_PACKET -> {
          WebSocketMessagePacket webSocketMessagePacket = gson.fromJson(
            wsMessageContext.message(),
            WebSocketMessagePacket.class
          );
          if (
            webSocketMessagePacket.getMessage().isResource() &&
            !KatServer.KatTokenPoolAPI.checkToken(
              webSocketMessagePacket.getMessage().resourceToken
            )
          ) {
            wsMessageContext.send(
              gson.toJson(
                ErrorPacket.builder().error("Invalid Resource Token").build()
              )
            );
            return;
          }
          ...
          // 处理消息发送事件
          KatServer.KatEventBusAPI.callEvent(
            MessageSendEvent
              .builder()
              .extensionId(webSocketMessagePacket.getMessage().extensionID)
              .message(webSocketMessagePacket.getMessage())
              .build()
          );
        ...
        case KatPacketTypeConstants.MESSAGE_QUERY_PACKET -> {
          WebsocketMessageQueryPacket websocketMessageQueryPacket = gson.fromJson(
            wsMessageContext.message(),
            WebsocketMessageQueryPacket.class
          );
          // 处理消息查询并返回
          Optional<List<KatUniMessage>> messages = KatMessageStorage.searchMessage(
            websocketMessageQueryPacket.getExtensionId(),
            websocketMessageQueryPacket.getMessageGroup(),
            websocketMessageQueryPacket.getStartTimeStamp(),
            websocketMessageQueryPacket.getEndTimeStamp()
          );
        ...
        case KatPacketTypeConstants.CUSTOM_PACKET -> {
          CustomPacket customPacket = gson.fromJson(
            wsMessageContext.message(),
            CustomPacket.class
          );
          KatServer.KatEventBusAPI.callEvent(
            CustomPacketReceiveEvent
              .builder()
              .extensionId(customPacket.getExtensionId())
              .content(customPacket.getContent())
              .build()
          );
        }
      }
      wsMessageContext.send(
        gson.toJson(ErrorPacket.builder().error("Unknown Packet").build())
      );
    });
  }
}
